...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, default='resnet18',  choices=['resnet18', "vgg16", "vit-s-16", "swin-t", "pre_resnet18"], help='model name')
    parser.add_argument('--exps_dir', type=str, default="/data/khw/workshop/boundary_unlearn/classification/exps", help='experiments directory')
    ##########################################################################################################
    ...
    parser.add_argument('--optim_name', type=str, default='sgd', choices=['sgd', 'adamw'], help='optimizer name')
    ##########################################################################################################
    ...
    parser.add_argument('--k', type=float, default=1.0, help='for ldmu, the number of lowest classes to use')  # 只用于ldmu方法，其他方法不使用
    ...
    ############################################# 默认参数，不需要任何调整 #######################################
    parser.add_argument('--extra_exp', type=str, help='optional extra experiment for boundary shrink', default=None, choices=['curv', 'weight_assign', None])
    ...
    parser.add_argument('--scaler',    type=float, default=None)
    ##########################################################################################################
    parser.add_argument("--fixed_class_set", action="store_true")  # 是否固定选择遗忘类别，默认不固定
    ...
    config_path_candidates = [f'config/{args.dataset_name}_{args.model_name}.yaml']
    if args.dataset_name == "tiny_imagenet" and args.model_name == "swin-t":
        config_path_candidates.append("config/tiny_imagenet_siwn-t.yaml")
    ...
    if config is None:
        raise FileNotFoundError(f"Config file not found. Tried: {', '.join(config_path_candidates)}")
    ...
    description = f"{args.seed}_{args.dataset_name}_{model_name}_forget{forget_class}" # 只用于unlearn方法，pretrain不需要 
    ...
    path = Path(args.exps_dir)   # 保存日志、结果、模型、配置的根目录
    create_dir(path)
    ...
    if args.fixed_class_set:
        pass
    else:
        random.shuffle(permutation_map)
    ...
    if args.train_from_scratch or args.retrain_from_scratch:
        ckpt_path = path/ "pretrained_model"   # 防止不小心跑的覆盖了原来的模型
    ...
    if args.train_from_scratch:  # original 和 retrain 不使用description， 其他的unlearn使用
        ...
        if args.dataset_name == "vggface":
            ...
            ori_model = train_save_model(train_loader, test_loader, model_name, args.optim_name, args.pretrain_lr,
                                        args.pretrain_epoch, ckpt_path, f"{args.seed}_{args.dataset_name}_{model_name}_original_model")
    ...
    if args.retrain_from_scratch:
        ...
        if args.dataset_name == "vggface":
            ...
            retrain_model = train_save_model(train_remain_loader, test_remain_loader, model_name, args.optim_name, args.pretrain_lr, 
                                            args.pretrain_epoch, ckpt_path, f"{args.seed}_{args.dataset_name}_{model_name}_retrain_forget{forget_class}_model")
    ...
    if args.load_original_model_path:
        ...
        original_model_path = ckpt_path / f'{args.seed}_{args.dataset_name}_{model_name}_original_model.pth'
    ...
    if args.load_retrain_model_path:
        ...
        retrain_model_path = ckpt_path / f'{args.seed}_{args.dataset_name}_{model_name}_retrain_forget{forget_class}_model.pth'
    ...
    if args.method == "random_label":
        ...
        if model_name=="pre_resnet18":
            bad_teacher_model = get_model("resnet18", num_classes).to("cuda")
        elif model_name=="swin-t":
            bad_teacher_model = get_model("bad-swin-t", num_classes).to("cuda")
        else:
    ...
    elif args.method == 'ldmu':
        unlearn_model = method.ldmu(args.k, ori_model, train_forget_loader,
                                                    args.unlearn_epoch, args.unlearn_rate,
                                                    logger=logger, console_handler=console_handler,
                                                    loader_dict=loader_dict, experiment_path= experiment_path, disable_bn = disable_bn,
                                                    ############## 我的额外自定义参数开始
        ...
        # args.scaler = SSD paper alpha
        # args.damp   = SSD paper lambda

        unlearn_model = method.ssd(
            ori_model,
            train_loader,
            train_remain_loader,
            train_forget_loader,
            test_remain_loader,
            test_forget_loader,
            dampening_constant=args.damp,
            selection_weighting=args.scaler,
    ...
    #path 고치기 
    path_ckpt = Path("model_checkpoint")
    path_ckpt.mkdir(parents=True, exist_ok=True)
    formatted_time = now.strftime("%m%d-%H%M%S")
    if args.method == 'ssd':
        ckpt_name = f"{args.seed}_{args.dataset_name}_{model_name}_forget{forget_class}_{args.method}_damp{args.damp}_scaler{args.scaler}_{formatted_time}.pth"
    else:
        ckpt_name = f"{args.seed}_{args.dataset_name}_{model_name}_forget{forget_class}_{args.method}_{formatted_time}.pth"

    if unlearn_model:
        # torch.save(unlearn_model.state_dict(), path / description / vice_description / f"ckpt.pth")
        #고치기
        torch.save(unlearn_model, path_ckpt / ckpt_name)
